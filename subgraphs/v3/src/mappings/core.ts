...
export function handleSwap(event: SwapEvent): void {
  ...
  // NEW: Calculate USD price per token for candlestick charts
  let token0PriceUSD = ZERO_BD;
  let token1PriceUSD = ZERO_BD;

  if (amount0Abs.gt(ZERO_BD)) {
    token0PriceUSD = amount0USD.div(amount0Abs);
  }
  if (amount1Abs.gt(ZERO_BD)) {
    token1PriceUSD = amount1USD.div(amount1Abs);
  }

  // NEW: Determine if this involves stable tokens
  let isStablePool = false;
  let stableTokens = [
    "0x203a662b0bd271a6ed5a60edfbd04bfce608fd36", // USDC (lowercase)
    "0x2dca96907fde857dd3d816880a0df407eeb2d2f2", // USDT (lowercase)
  ];

  for (let i = 0; i < stableTokens.length; i++) {
    if (
      token0.id.toLowerCase() == stableTokens[i] ||
      token1.id.toLowerCase() == stableTokens[i]
    ) {
      isStablePool = true;
      break;
    }
  ...
  // NEW: Add USD price data for candlestick charts
  swap.token0PriceUSD = token0PriceUSD;
  swap.token1PriceUSD = token1PriceUSD;
  swap.amount0USD = amount0USD;
  swap.amount1USD = amount1USD;
  swap.isStablePool = isStablePool;
...
function loadTickUpdateFeeVarsAndSave(
  tickId: i32,
